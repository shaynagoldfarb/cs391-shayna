...
def term_comp00(tm0):
    ...
    if tm0.ctag == "TMopr":
        ...
        if (pnm == "-"):
            assert len(ags) == 2
            res = tv1
            for _ in range(chnum_toint(term_eval00(tv2))):
                res = term_app(chnum_pre, res)
            return res
        if (pnm == "*"):
            return term_app(term_app(chnum_mul, tv1), tv2)
        if (pnm == "<="):
            tmp = tv2
            for _ in range(chnum_toint(term_eval00(tv1))):
                tmp = term_app(chnum_pre, tmp)
            return term_app(chnum_gtz, tmp)
